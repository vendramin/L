...
def construct_diamond(n):
    ...
    for k in range(1,case_number+1):
        print(f"Case {k} of {case_number}")
        time1 = time.time()
        os.system(f'savilerow ./diamond.eprime ./diamond{n}_{k}.param -run-solver -all-solutions -solutions-to-stdout-one-line > diamond{n}_{k}.output')

        os.system(f'bzip2 diamond{n}_{k}.output')
